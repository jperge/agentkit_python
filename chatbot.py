...
def setup():
    ...
    # Validate required CDP API credentials
    ...
    if not api_key_id or not api_key_secret:
        print("Error: CDP_API_KEY_ID and CDP_API_KEY_SECRET must be set.")
        print("Get them from: https://portal.cdp.coinbase.com/access/api")
        sys.exit(1)

    # Sanitize the secret to handle escaped newlines common in .env files
    api_key_secret = api_key_secret.replace('\\n', '\n')

    # Sanitize wallet secret if provided
